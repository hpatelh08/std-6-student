...
const BookReaderPage: React.FC<BookReaderPageProps> = ({ book, onBack }) => {
  ...
  /* ── Reading Tracker ────────────────────────── */
  const tracker = useReadingTracker(book.id, book.title, numPages);
  const [activitiesExpanded, setActivitiesExpanded] = useState(false);

  const handleBack = useCallback(() => {
    pauseReadTimer();
    onBack();
  }, [pauseReadTimer, onBack]);

  useEffect(() => {
    startReadTimer();
    return () => { pauseReadTimer(); };
  }, [startReadTimer, pauseReadTimer]);
  ...
  }, [readLimitEnabled, readIsExpired, pauseReadTimer, onBack]);
  ...
  const dimensions = useMemo(() => {
    ...
    const bottomExtra = isFullscreen ? 5 : (BOTTOM_BAR_H + 16);
  ...
  useEffect(() => {
    if (displayPage >= 1 && displayPage <= numPages) {
      tracker.trackPageView(displayPage);
    }
  }, [displayPage, numPages, tracker.trackPageView]);
  ...
  }, [handleBack, selectedWord, askAIOpen, askAIMinimized, showStats, flipNext, flipPrev]);
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          >
            ...
            {pdfError && (
              <motion.div style={{ textAlign: 'center', maxWidth: 320, padding: '0 24px' }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <motion.button onClick={handleBack} style={{
          ...
          {false && !isLoading && !pdfError && displayPage >= 1 && displayPage <= numPages && (
            <motion.div
              style={{
                flexShrink: 0,
                overflow: 'hidden',
                ...
                borderBottom: activitiesExpanded ? '1px solid rgba(99,102,241,0.08)' : 'none',
              }}
              initial={false}
              animate={{ maxHeight: activitiesExpanded ? 400 : 40 }}
            ...
            >
              ...
              >
                ...
                <motion.span
                  animate={{ rotate: activitiesExpanded ? 180 : 0 }}
              ...
              {activitiesExpanded && (
                <motion.div 
                  ...
                  transition={{ duration: 0.25 }}
                  style={{ 
                    padding: isMobile ? '0 8px 8px' : '0 24px 12px',
                    overflow: 'auto',
                    maxHeight: 360,
                  }}
                >
